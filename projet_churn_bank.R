...
summary(data)
summary(data$Attrition_Flag)

data_clean <- data[,-c(1,22,23)] # On supprime les 2 dernières colonnes inutiles
...
#plot("Attrition_Flag", "Customer_Age", data = data)
...
#Plot par genre
categ <- data_clean %>% select(Attrition_Flag,Gender, Marital_Status, Card_Category, Income_Category, Education_Level)
categ %>%
...
# supprimer IGNORER LES 2 DERNIÈRES COLONNES (NAIVE BAYES CLAS…)
