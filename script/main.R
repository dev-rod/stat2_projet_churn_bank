...
if("grid" %in% rownames(installed.packages()) == FALSE) {install.packages("grid",dependencies=TRUE)};library(grid)

###### test exercice datanovia
# data("iris")
# df <- iris
# head(df, 3)
# # Calculer k-means avec k = 3 après Suppression de la 5ème colonne (Species) et 
# # normalisation des données (scale) pour rendre les variables comparables
# # Calculer le clustering k-means en utilisant k = 3.
# # Comme le résultat final de la classification k-means est sensible aux assignations initiales aléatoires,
# # nous spécifions nstart = 25. Cela signifie que R va essayer 25 affectations différentes de départ aléatoire
# # et ensuite sélectionner les meilleurs résultats.
# set.seed(123)
# res.km <- kmeans(scale(df[, -5]), 3, nstart = 25)
# # Clustering K-means montrant le groupe de chaque individu
# res.km$cluster
# 
# factoextra::fviz_cluster(res.km, data = df[, -5],
...
# # Réduction de chacune des 4 variables quantitatives (exit qualitative species) à 4 groupe/dimension en utilisant l'ACP(prcomp)
# res.pca <- prcomp(df[, -5], scale = TRUE)
# # Coordonnées sur les 4 dimensions de chaque individus
...
# PCA KMEANS trouvé sur Kaggle => en doc "who's gonna churn?"

# trouver 2 types de clients pour avoir un équivalent de ce texte
...
# Ensemble de données de clustering en 6 dimensions, nous pouvons voir que la 4ème dimension couvre
# environ 50% du total des clients en rotation, cela pourrait signifier que la 4ème dimension du clustering
...
# environ 27% de la variance totale des données"
...
library(gridExtra)
gridExtra::grid.arrange(p1, p2, ncol=2,top=my_top)
